...
const LOC = {
  ...
  // Погрузка Поляны подтверждена (владелец, 30.07.2026): «ул. Красная Сосна, д. 30, с. 1».
  // В справочнике у LOC-POLYANA стоит «ул. Докукина, д. 10с11» — адрес в реестре нужно
  // ИСПРАВИТЬ на Красную Сосну (резолв идёт по коду LOC-POLYANA, но адрес расходится).
...
export const IMPORT_RULES: Record<string, ImportRule> = {
  ...
  'korolevsky-vkus': {
    ...
    brandMap: {
      ...
      'перспектива': 'PERSPECT',          // заведён 30.07.2026 (SHIPPER, вертикаль LAAS-LTL)
    },
