...
function groupByLocations(tariffs: any[]): RouteData[] {
  const byLoc = new Map<string, { originLocation: any; destinationLocation: any; byDate: Map<string, any> }>();
  for (const t of tariffs) {
    const rk = `${t.originLocationId || '__'}_${t.destinationLocationId || '__'}`;
    if (!byLoc.has(rk)) byLoc.set(rk, { originLocation: t.originLocation, destinationLocation: t.destinationLocation, byDate: new Map() });
    const entry = byLoc.get(rk)!;
    const dk = dayjs(t.validFrom).format('YYYY-MM-DD');
    if (!entry.byDate.has(dk)) entry.byDate.set(dk, t);
  }
  const result: RouteData[] = [];
  for (const [, { originLocation, destinationLocation, byDate }] of Array.from(byLoc)) {
